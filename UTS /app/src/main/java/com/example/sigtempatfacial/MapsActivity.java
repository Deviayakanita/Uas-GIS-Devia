...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private String[] nama,alamat,no_tlpn,gambar,id;
    int jumData;
    ...
    private void getlokasi(){
        String url = "https://facialskinbalii.000webhostapp.com/facialskin.php";
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        for (int i = 0;  i < jumData; i++) {
                            try {
                                ...
                                no_tlpn[i] = data.getString("no_tlpn");
                            ...
                            public boolean onMarkerClick(Marker marker) {
                                for (int i=0;i<jumData;i++) {
                                    if (marker.getTitle().equals(nama[i])){
                                        if (MarkerD[i]){
                                            Detailfacial_Activity.id= id[i];
                                            Detailfacial_Activity.nama = nama[i];
                                            Detailfacial_Activity.alamat = alamat[i];
                                            Detailfacial_Activity.no_tlpn = no_tlpn[i];
                                            Detailfacial_Activity.gambar = gambar[i];
                                            Detailfacial_Activity.latitude = latitude[i];
                                            Detailfacial_Activity.longitude = longitude[i];
                                            Intent pindahdetail = new Intent(MapsActivity.this,Detailfacial_Activity.class);
                                            ...
                                            Toast pesan = Toast.makeText(MapsActivity.this,"Silakan Klik Sekali Lagi Disini",Toast.LENGTH_LONG);
